    ...
    <div x-data="{open: false}" class="mb-3">
        ...
        <div x-show="open" @click.away="open = false" class="grid grid-cols-6 gap-6">
            <div class="col-span-6 sm:col-span-3">
                ...
                <input type="text" wire:model="organization" name="organization" id="organization" value="{{ $venue->organization }}" class="mt-1 focus:ring-rojo focus:border-rojo block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
            </div>
            <div class="col-span-6 sm:col-span-3">
                <label for="address" class="block text-sm font-medium text-gray-700">Address</label>
                <input type="text" wire:model="address" name="address" id="address" value="{{ $venue->address }}" class="mt-1 focus:ring-rojo focus:rojo block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
            </div>
            <div class="col-span-6 sm:col-span-3">
                <label for="city" class="block text-sm font-medium text-gray-700">City</label>
                <input type="text" wire:model="city" name="city" id="city" value="{{ $venue->city }}" class="mt-1 focus:ring-rojo focus:rojo block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
            </div>
            <div class="col-span-6 sm:col-span-3">
                <label for="description" class="block text-sm font-medium text-gray-700">Description</label>
                <textarea type="text" wire:model="description" name="description" id="description" value="{{ $venue->description }}" class="mt-1 focus:ring-rojo focus:border-rojo block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"></textarea>
            </div>
            <div class="col-span-6 sm:col-span-3">
                <label for="phone" class="block text-sm font-medium text-gray-700">Phone</label>
                <input type="text" wire:model="phone" name="phone" id="phone" value="{{ $venue->phone }}" class="mt-1 focus:ring-rojo focus:border-rojo block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
            </div>
            <div class="col-span-6 sm:col-span-3">
        ...
        <div class="mt-4">
            ...
            @if ($yellowCups > 0)
                @for ($i = 0; $i < $yellowCups; $i++)
                    <i class="fas fa-glass-martini fa-3x text-rojo"></i>
                    @php
                    $count-- 
                    @endphp
                @endfor    
